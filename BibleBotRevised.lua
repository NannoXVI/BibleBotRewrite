...
local function findPlayer(nameOrDisplayName)
    ...
    for k, v in pairs(game:GetService('Players'):GetPlayers()) do
        if _.includes(v.Name:lower(), nameOrDisplayName:lower()) or _.includes(v.DisplayName:lower(), nameOrDisplayName:lower()) then
            return v
        end
    end
end
...
local TimeBetweenChatSlider = page.Slider({
...
local welcomingButton = page.Toggle({
...
local blacklistButton = page.Button({
...
local function askGod()
    local ans = {
        ...
        "A pity, made in my image yet couldn't ask a more reasonable question for me...";"Such foul words, I am ashamed of you";"Think twice of what you ask of me."; "Yes"; "No";
...
-- setup on chat event
game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:Connect(function(object) 
    if isUserBlocked(object.FromSpeaker) then 
        print("Blocked user " .. object.FromSpeaker .. " attempt from using bible bot commands")
        return
    end
...
Players.PlayerAdded:Connect(function(NewPlayer)
    if not welcomingEnabled then return end
